...
from .util import transform_chw, transform_chw_single
...
class NYU_Depth_V2(data.Dataset):
    def __init__(self, root, split='test', transform=None, limit=None, debug=False):
        ...
        if debug:
            ...
            folder = os.path.join(root, 'npy')
            self.images = np.load(os.path.join(folder, 'test_images.npy'))
            self.depths = np.load(os.path.join(folder, 'test_depths.npy'))
        else:
            folder = os.path.join(root, 'npy')
            self.images = np.load(os.path.join(folder, 'test_images.npy'))
            self.depths = np.load(os.path.join(folder, 'test_depths.npy'))
    ...
    def __getitem__(self, index):
        ...
        #   image = transform_chw_single(self.transform, image)
        #   depth = transform_chw_single(self.transform, depth)
        return image, depth
